...
const login = async (payload: { email: string, password: string }) => {
    ...
    const accessToken = jwtHelper.generateToken({ id: user.id, email: user.email, role: user.role }, config.jwt.access_token_secret, config.jwt.access_token_expires_in);

    const refreshToken = jwtHelper.generateToken({ id: user.id, email: user.email, role: user.role }, config.jwt.refresh_token_secret, config.jwt.refresh_token_expires_in);
...
const changePassword = async (user: any, payload: any) => {
    ...
    const hashedPassword: string = await bcrypt.hash(payload.newPassword, Number(config.salt_rounds));
